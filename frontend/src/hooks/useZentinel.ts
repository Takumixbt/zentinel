...
import { ZENTINEL_ABI, ZENTINEL_ADDRESS, CHAIN_ID, isZeroHandle } from "@/lib/contract";
...
export function useZentinel() {
  ...
  const [phase, setPhase] = useState<"idle" | "encrypting" | "submitting" | "computing" | "revealing" | "decrypting">(
    "idle",
  );
  ...
  const settleLog = useCallback((id: number, kind: ActivityKind, text?: string, detail?: string) => {
    setLog((l) => l.map((a) => (a.id === id ? { ...a, kind, text: text ?? a.text, detail: detail ?? a.detail } : a)));
  }, []);
  ...
  const revealVerdict = useCallback(async () => {
    ...
    setErrorMsg(null);
    const id = pushLog("pending", "Requesting public decryption from Zama's KMS…", "This can take ~15–20s");
    try {
      setPhase("revealing");
      const handle = (await publicClient.readContract({
        address: ZENTINEL_ADDRESS,
      ...
      for (let attempt = 0; attempt < 6; attempt++) {
        try {
          ...
          const v = lookupHandle(res.clearValues as unknown as Record<string, unknown>, handle);
  ...
  const revealMyPosition = useCallback(async () => {
    if (!ready || !instance || !address || !publicClient) return;
    setErrorMsg(null);
    const id = pushLog("pending", "Decrypting your private figures…", "Sign the EIP-712 request in your wallet");
    try {
      setPhase("decrypting");
      ...
      const eip712 = instance.createEIP712(keypair.publicKey, [ZENTINEL_ADDRESS], startTimestamp, durationDays);
  ...
  const steps = useMemo(() => {
    ...
    const submitStep: StepState = !connectDone ? "idle" : submittedDone ? "done" : "active";
    const computeStep: StepState = !submittedDone ? "idle" : computedDone ? "done" : "active";
    const revealStep: StepState = !computedDone ? "idle" : verdict !== null ? "done" : "active";
    return {
