...
export function Stepper({ states }: { states: Record<string, StepState> }) {
  return (
        ...
        {STEPS.map((step, i) => {
          ...
          return (
            <li
              key={step.key}
              className={cn(
                "relative flex items-center gap-3 border px-3 py-3 transition-colors duration-300",
            ...
            >
              <div
                className={cn(
                  "flex h-9 w-9 shrink-0 items-center justify-center border text-sm font-semibold transition-all duration-300",
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="text-[0.62rem] font-mono text-content-dim">0{i + 1}</span>
                  <span
                    className={cn(
                      "truncate text-sm font-medium",
                      done ? "text-safe" : active ? "text-content" : "text-content-muted",
                    )}
                  >
                    {step.label}
                  </span>
