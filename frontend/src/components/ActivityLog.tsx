...
export function ActivityLog({ items }: { items: Activity[] }) {
  ...
  return (
    <div className="border border-hairline bg-surface-inset/50 p-3">
              ...
              a.kind === "success"
                ...
                : a.kind === "error"
                  ...
                  : a.kind === "pending"
                    ...
                    : "text-content-muted";
            return (
              <div key={a.id} className="animate-fade-up flex items-start gap-2.5 px-2 py-1.5">
                <Icon className={cn("mt-0.5 h-4 w-4 shrink-0", color, a.kind === "pending" && "animate-spin")} />
                <div className="min-w-0">
                  <div className={cn("text-sm leading-tight", color)}>{a.text}</div>
                  {a.detail && <div className="truncate font-mono text-[0.68rem] text-content-dim">{a.detail}</div>}
                </div>
              </div>
            );
          })
